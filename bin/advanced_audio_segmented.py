...
def convertMP3toWav(fname, bitrate):
    file_path_output = ".".join(fname.split(".")[:-1] + ["wav"])
    cmd = "ffmpeg -i {} -codec:a pcm_s16le -ac 1 -ar {} {}".format(fname, bitrate, file_path_output)
    print(cmd)
    os.system(cmd)
    return file_path_output
...
def get_split_start_ends_with_max_size(split_start, split_end, max_size=60):
    split_starts = []
    split_ends = []
    curr_start = split_start
    while True:
        split_starts.append(curr_start)
        split_ends.append(min(split_end, curr_start + max_size))
        curr_start += max_size
        if curr_start >= split_end:
            break
...
def SplitWavFileIntoSeconds_GoingBack(filename, split_length_inSeconds, silence_Period, file_period_dic):
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            raise
    change_file_name = os.path.join("tmp_segments", file_dir)
    """
    os.system("rm -rf tmp_segments")
    os.system("mkdir tmp_segments")

    split_end = 0
    split_start = 0
    i = 0
    tokens = getSplitAudioDurationListBetweenSilence(filename,split_length_inSeconds,silence_Period)
    fw = open("tmp_segments/timestamps.txt", "w")
    for j, t in tqdm(enumerate(tokens)):
        ...
        split_starts, split_ends = get_split_start_ends_with_max_size(split_start, split_end, max_size=60)

        for split_start,split_end in zip(split_starts, split_ends):
            utt_id = "{:08d}".format(i)
            seg_name = os.path.join("tmp_segments", "{}-{}-{}".format(utt_id, "speaker1", utt_id))
            fw.write("{} {} {}\n".format(os.path.basename(seg_name), split_start, split_end))
            cmd = "ffmpeg -i {} -ss {} -to {} -c:a copy {}.{}".                format(filename, split_start, split_end, seg_name, ext)
            #print("About to run: {}".format(cmd))
            check_call(shlex.split(cmd), universal_newlines=True)
            if (split_start == 0):
...
def getSplitAudioDurationListBetweenSilence(fileName,eachAudioLen,silencePeriod,energyThreshold=55):
    try:
        ...
        validator = AudioEnergyValidator(sample_width=asource.sample_width, energy_threshold=energyThreshold, channels=1)
...
if __name__ == '__main__':
    ...
    ##### MP3 to WAV to SEGMENTS #####
    for fname in tqdm(args.fileList):
        print(fname)
        file_period_dic = {}
        SplitfileOutputPath = SplitWavFileIntoSeconds_GoingBack(convertMP3toWav(fname, audioBitRateInKbps),split_length_inSeconds,rewindAudio, file_period_dic)
        print("main caller {0}".format(file_period_dic))
        print(SplitfileOutputPath)
        break
